...
try {
    ...
    if ($readinessBody.status -ne 'not-ready' -or
        ...
        [int]$readinessBody.missingDependencyCount -ne 54) {
        throw 'Readiness response did not disclose the expected 54 fail-closed runtime dependencies.'
    }
    ...
    if ($internalServiceBody.productId -ne 'sovereign-internal-services' -or
        $internalServiceBody.increment -ne 'Operational Increment 09 - Governed Code Generation Candidate' -or
    ...
    if ($openApiBody.openapi -ne '3.1.0' -or
        ...
        -not $openApiBody.paths.'/api/v1/internal-services/intents/{registrationId}/enterprise-context/{contextDiscoveryId}/existing-systems/{systemsDiscoveryId}/existing-architecture/{architectureDiscoveryId}/approved-packages/{packageSelectionId}/ai-planning/{planningId}/code-generation') {
        throw 'Runtime OpenAPI response does not contain the approved path through Code Generation.'
    }
    ...
    Write-Output 'RUNTIME VERIFIED: Development API live, 54 runtime dependencies fail-closed, and Governed Code Generation remains protected without model, source-write, or context adapters.'
}
finally {
