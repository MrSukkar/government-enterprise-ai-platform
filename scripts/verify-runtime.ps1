...
try {
    ...
    if ($readinessBody.status -ne 'not-ready' -or
        ...
        [int]$readinessBody.missingDependencyCount -ne 148) {
        throw 'Readiness response did not disclose the expected 148 fail-closed runtime dependencies.'
    }
    if ($readinessBody.controlPlanes.identity -ne 'unconfigured' -or
        ...
        $readinessBody.controlPlanes.sandbox -ne 'unconfigured' -or $readinessBody.controlPlanes.tests -ne 'unconfigured') {
    ...
    Write-Output 'RUNTIME VERIFIED: Development API live; all control planes through Tests remain safely unconfigured; 148 runtime dependencies fail closed; the approved Create Internal Service contract remains complete through Evidence.'
}
finally {
