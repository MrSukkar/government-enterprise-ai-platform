...
try {
    ...
    if ($readinessBody.status -ne 'not-ready' -or
        ...
        [int]$readinessBody.missingDependencyCount -ne 151) {
        throw 'Readiness response did not disclose the expected 151 fail-closed runtime dependencies.'
    }
    if ($readinessBody.controlPlanes.identity -ne 'unconfigured' -or
        ...
        $readinessBody.controlPlanes.humanReview -ne 'unconfigured') {
...
Write-Output 'RUNTIME VERIFIED: Development API live; all control planes through Human Review remain safely unconfigured; 151 runtime dependencies fail closed; the approved Create Internal Service contract remains complete through Evidence.'
}
finally {
