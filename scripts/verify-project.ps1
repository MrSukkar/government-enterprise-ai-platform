...
if (Test-Path $stage18AcceptancePath) {
    ...
    $page=Get-Content -Raw $stage18Paths.Page;@('Integration Demo Stage 18','Activate governed OpenTelemetry','ActivateTelemetryAsync','operator.internal-service.telemetry.activate','GovernedTelemetrySignal.Traces','TelemetryConfigured: true','AutomaticRegistrationOccurred: false','CanAdvance: false','geaip-demo-stage-18')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 18 UI guard missing: $_"}}
