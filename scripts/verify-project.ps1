...
$wave21AcceptancePath=Join-Path $repositoryRoot 'docs\operationalization\WAVE_21_ACCEPTANCE.md'
if(Test-Path $wave21AcceptancePath){
 $paths=@{Change=Join-Path $repositoryRoot 'docs\change-control\CR-022-OPERATIONALIZATION-WAVE-21.md';Master=Join-Path $repositoryRoot 'docs\PROJECT_MASTER_SPECIFICATION_V2.md';Envelope=Join-Path $repositoryRoot 'backend\Platform.Governance\Policies\ISovereignPolicyEvaluationClient.cs';Engine=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\InternalServices\GovernedEvidenceCompletion.cs';Ops=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\InternalServices\EvidenceCompletionOperationalization.cs';Persistence=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\Persistence\PostgreSqlEvidenceCompletionOperationalization.cs';Inputs=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\Persistence\Migrations\035_evidence_completion_inputs.sql';Chain=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\Persistence\Migrations\036_evidence_chain.sql';Services=Join-Path $repositoryRoot 'backend\Platform.SoftwareFactory\SoftwareFactoryServiceCollectionExtensions.cs'};$paths.Values|ForEach-Object{if(-not(Test-Path $_)){throw "Wave 21 artifact missing: $_"}}
 $change=Get-Content -Raw $paths.Change;@('Status: **Approved for Operationalization Wave 21**','Decision: **Approved by the repository owner','No evidence update/deletion')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Wave 21 approval missing: $_"}}
 $envelope=Get-Content -Raw $paths.Envelope;@('SovereignEvidenceCompletionPolicyScope','AppendFinalEvidenceAllowed','VerificationAllowed','WorkflowAdvancementAllowed','EvidenceCompletion')|ForEach-Object{if($envelope-notmatch[regex]::Escape($_)){throw "Evidence completion policy envelope missing: $_"}}
 $engine=Get-Content -Raw $paths.Engine;@('CryptographicEvidenceEngine','EvidenceStage.Evidence','AppendAsync','VerifyAsync','EntryProofs.Length','SignatureValid','IsComplete','VerticalSliceComplete')|ForEach-Object{if($engine-notmatch[regex]::Escape($_)){throw "Evidence completion engine missing: $_"}}
 $ops=Get-Content -Raw $paths.Ops;@('SovereignEvidenceCompletionPolicyGate','internal-service.evidence.complete','DeterministicEvidenceAccessAuthorizer','SovereignEvidenceSigner','SovereignEvidenceSignatureVerifier','RS256','ES256','DeterministicEvidenceCompletionResultAuthorizer')|ForEach-Object{if($ops-notmatch[regex]::Escape($_)){throw "Evidence completion operationalization missing: $_"}}
 $p=Get-Content -Raw $paths.Persistence;@('IAuthorizedEnterpriseModelContextualizationReceiptReader','IEvidenceCompletionDeliveryRunReader','IEvidenceChainStore','IsolationLevel.Serializable','ON CONFLICT DO NOTHING','FOR UPDATE','LoadOrderedAsync')|ForEach-Object{if($p-notmatch[regex]::Escape($_)){throw "Evidence completion persistence missing: $_"}};if($p-match'UPDATE evidence|DELETE FROM evidence'){throw 'Evidence chain persistence exposes update or delete.'}
 $chain=Get-Content -Raw $paths.Chain;@('evidence.chain_entries','PRIMARY KEY(tenant_id,chain_id,sequence)','REVOKE UPDATE,DELETE,TRUNCATE')|ForEach-Object{if($chain-notmatch[regex]::Escape($_)){throw "Evidence chain migration missing: $_"}}
 $services=Get-Content -Raw $paths.Services;@('EvidenceCompletionRuntimeReadiness','PostgreSqlEvidenceChainStore','DeterministicEvidenceAccessAuthorizer','SovereignEvidenceSigner','SovereignEvidenceSignatureVerifier','DeterministicEvidenceCompletionResultAuthorizer')|ForEach-Object{if($services-notmatch[regex]::Escape($_)){throw "Evidence completion composition missing: $_"}}
...
$stage05AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_05_EXISTING_ARCHITECTURE_ACCEPTANCE.md'
if (Test-Path $stage05AcceptancePath) {
    $stage05Paths = @{
        Change = Join-Path $repositoryRoot 'docs\change-control\CR-027-INTEGRATION-DEMO-EXISTING-ARCHITECTURE.md'
        Compose = Join-Path $repositoryRoot 'deploy\demo\stage-05\compose.yml'
        Policy = Join-Path $repositoryRoot 'deploy\demo\stage-05\opa\decision.rego'
        Seed = Join-Path $repositoryRoot 'deploy\demo\stage-05\neo4j\seed.cypher'
        Preparation = Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-05.ps1'
        Page = Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'
    }
    $stage05Paths.Values | ForEach-Object { if (-not (Test-Path -LiteralPath $_)) { throw "Stage 05 artifact missing: $_" } }
    $change = Get-Content -Raw $stage05Paths.Change
    @('Status: **Approved for bounded implementation**','Component','Interface','CanAdvance','Phase 31') | ForEach-Object { if ($change -notmatch [regex]::Escape($_)) { throw "Stage 05 approval missing: $_" } }
    $compose = Get-Content -Raw $stage05Paths.Compose
    @('004_existing_architecture_evidence.sql','neo4j:2025.10-community@sha256:','127.0.0.1:7687') | ForEach-Object { if ($compose -notmatch [regex]::Escape($_)) { throw "Stage 05 composition guard missing: $_" } }
    $policy = Get-Content -Raw $stage05Paths.Policy
    @('internal-service.existing-architecture.discover','Component','Interface','DependsOn','enterprise-graph','Developer','"Deny"') | ForEach-Object { if ($policy -notmatch [regex]::Escape($_)) { throw "Stage 05 policy guard missing: $_" } }
    $seed = Get-Content -Raw $stage05Paths.Seed
    @('EnterpriseArchitecture','Approved','Active','generatedContentIncluded=false','externalEffectOccurred=false') | ForEach-Object { if ($seed -notmatch [regex]::Escape($_)) { throw "Stage 05 seed guard missing: $_" } }
    $page = Get-Content -Raw $stage05Paths.Page
...
if (Test-Path $stage08AcceptancePath) {
    ...
    @('Create governed code candidate','CreateCodeCandidateAsync','developer.internal-service.code-generation.create','IsExecutable: false','IsApplied: false','CanAdvance: false') | ForEach-Object { if ($page -notmatch [regex]::Escape($_)) { throw "Stage 08 UI guard missing: $_" } }
...
$stage09AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_09_STATIC_VALIDATION_ACCEPTANCE.md'
if (Test-Path $stage09AcceptancePath) {
    $stage09Paths = @{
        Change = Join-Path $repositoryRoot 'docs\change-control\CR-031-INTEGRATION-DEMO-STATIC-VALIDATION.md'
        Compose = Join-Path $repositoryRoot 'deploy\demo\stage-09\compose.yml'
        Policy = Join-Path $repositoryRoot 'deploy\demo\stage-09\opa\decision.rego'
        Bundle = Join-Path $repositoryRoot 'deploy\demo\stage-09\opa\bundle_verification.rego'
        Preparation = Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-09.ps1'
        Runbook = Join-Path $repositoryRoot 'deploy\demo\stage-09\README.md'
        Page = Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'
        Realm = Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json'
    }
    $stage09Paths.Values | ForEach-Object { if (-not (Test-Path -LiteralPath $_)) { throw "Stage 09 artifact missing: $_" } }
    $change = Get-Content -Raw $stage09Paths.Change
    @('Status: **Approved for bounded implementation**','deterministic','signed','non-executable','Security Validation','Phase 31') | ForEach-Object { if ($change -notmatch [regex]::Escape($_)) { throw "Stage 09 approval missing: $_" } }
    $compose = Get-Content -Raw $stage09Paths.Compose
    @('011_static_validation_inputs.sql','012_static_validation_evidence.sql','integration-demo-stage-09','127.0.0.1:8181','--verification-key') | ForEach-Object { if ($compose -notmatch [regex]::Escape($_)) { throw "Stage 09 composition guard missing: $_" } }
    $policy = Get-Content -Raw $stage09Paths.Policy
    @('internal-service.static-validation.create','demo-static-contract-shape','demo-static-source-boundary','"outputKind":"static-report"','"Deny"') | ForEach-Object { if ($policy -notmatch [regex]::Escape($_)) { throw "Stage 09 policy guard missing: $_" } }
    $preparation = Get-Content -Raw $stage09Paths.Preparation
    @('code-generation\seed.sql','--signing-key','Stage 09 infrastructure startup failed') | ForEach-Object { if ($preparation -notmatch [regex]::Escape($_)) { throw "Stage 09 preparation guard missing: $_" } }
...
if (Test-Path $stage15AcceptancePath) {
    ...
    @('Execute governed CI/CD','ExecuteCiCdAsync','developer.internal-service.cicd.execute','CiCdExpectedWorkflowSha256Digest','ArtifactPublished: false','DeploymentOccurred: false','CanAdvance: false','GovernedCiCdExecutionReceipt') | ForEach-Object { if ($page -notmatch [regex]::Escape($_)) { throw "Stage 15 UI guard missing: $_" } }
...
$stage16AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_16_GOVERNED_ARTIFACT_ACCEPTANCE.md'
if (Test-Path $stage16AcceptancePath) {
    $stage16Paths = @{
        Change = Join-Path $repositoryRoot 'docs\change-control\CR-038-INTEGRATION-DEMO-GOVERNED-ARTIFACT.md'
        Compose = Join-Path $repositoryRoot 'deploy\demo\stage-16\compose.yml'
        Policy = Join-Path $repositoryRoot 'deploy\demo\stage-16\opa\decision.rego'
        BundleVerification = Join-Path $repositoryRoot 'deploy\demo\stage-16\opa\bundle_verification.rego'
        Preparation = Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-16.ps1'
        Runbook = Join-Path $repositoryRoot 'deploy\demo\stage-16\README.md'
        Page = Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'
        Realm = Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json'
    }
    $stage16Paths.Values | ForEach-Object { if (-not (Test-Path -LiteralPath $_)) { throw "Stage 16 artifact missing: $_" } }
    $change = Get-Content -Raw $stage16Paths.Change
    @('Status: **Approved for bounded implementation**','immutable coordinate','No fake artifact','all six mandatory supply-chain controls','Phase 31') | ForEach-Object { if ($change -notmatch [regex]::Escape($_)) { throw "Stage 16 approval missing: $_" } }
    $compose = Get-Content -Raw $stage16Paths.Compose
    @('025_artifact_inputs.sql','026_artifact_evidence.sql','integration-demo-stage-16','artifact/seed.sql','--verification-key') | ForEach-Object { if ($compose -notmatch [regex]::Escape($_)) { throw "Stage 16 composition guard missing: $_" } }
    $policy = Get-Content -Raw $stage16Paths.Policy
    @('internal-service.artifact.publish','pipelineManifestSha256Digest','artifactName','contentSha256Digest','registryId','requiredControls','overwriteAllowed":false','deploymentAllowed":false','"outputKind":"artifact-publication-result"','"Deny"') | ForEach-Object { if ($policy -notmatch [regex]::Escape($_)) { throw "Stage 16 policy guard missing: $_" } }
    $bundle = Get-Content -Raw $stage16Paths.BundleVerification
    @('geaip-demo-stage-16','1616161616161616161616161616161616161616161616161616161616161616','trust://demo/opa/demo-opa-signing-01') | ForEach-Object { if ($bundle -notmatch [regex]::Escape($_)) { throw "Stage 16 bundle verification missing: $_" } }
    $preparation = Get-Content -Raw $stage16Paths.Preparation
    @('--signing-key','accepted CI/CD evidence','CiCd-stopped run and pipeline-output manifest','immutable Artifact registry gateway','Stage 16 infrastructure startup failed') | ForEach-Object { if ($preparation -notmatch [regex]::Escape($_)) { throw "Stage 16 preparation guard missing: $_" } }
    $runbook = Get-Content -Raw $stage16Paths.Runbook
    @('IInstitutionalArtifactRegistryGateway','Repository defaults remain empty and fail closed','never substitute a fake manifest','ArtifactPublished: true','RegistryMutated: true','SourceMutationOccurred: false','DeploymentOccurred: false','ProductionEffectOccurred: false','CanAdvance: false') | ForEach-Object { if ($runbook -notmatch [regex]::Escape($_)) { throw "Stage 16 runbook guard missing: $_" } }
    $page = Get-Content -Raw $stage16Paths.Page
    @('Publish governed Artifact','PublishArtifactAsync','developer.internal-service.artifact.publish','ArtifactExpectedContentSha256Digest','RegistryMutated: true','DeploymentOccurred: false','CanAdvance: false','GovernedArtifactPublicationReceipt') | ForEach-Object { if ($page -notmatch [regex]::Escape($_)) { throw "Stage 16 UI guard missing: $_" } }
    $realm = Get-Content -Raw $stage16Paths.Realm
    @('developer.internal-service.cicd.execute','developer.internal-service.artifact.publish','demo.human-reviewer') | ForEach-Object { if ($realm -notmatch [regex]::Escape($_)) { throw "Stage 16 identity guard missing: $_" } }
    $acceptance = Get-Content -Raw $stage16AcceptancePath
    @('CR-038','exact accepted CI/CD evidence','No fake artifact','ArtifactPublished: true','RegistryMutated: true','DeploymentOccurred: false','ProductionEffectOccurred: false','CanAdvance: false','Deployment and all later stages remain outside this gate') | ForEach-Object { if ($acceptance -notmatch [regex]::Escape($_)) { throw "Stage 16 acceptance missing: $_" } }
}

$stage17AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_17_GOVERNED_DEPLOYMENT_ACCEPTANCE.md'
if (Test-Path $stage17AcceptancePath) {
    $stage17Paths = @{ Change=Join-Path $repositoryRoot 'docs\change-control\CR-039-INTEGRATION-DEMO-GOVERNED-DEPLOYMENT.md'; Compose=Join-Path $repositoryRoot 'deploy\demo\stage-17\compose.yml'; Policy=Join-Path $repositoryRoot 'deploy\demo\stage-17\opa\decision.rego'; BundleVerification=Join-Path $repositoryRoot 'deploy\demo\stage-17\opa\bundle_verification.rego'; Preparation=Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-17.ps1'; Runbook=Join-Path $repositoryRoot 'deploy\demo\stage-17\README.md'; Page=Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'; Realm=Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json' }
    $stage17Paths.Values|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){throw "Stage 17 artifact missing: $_"}}
    $change=Get-Content -Raw $stage17Paths.Change;@('Approved for bounded implementation','explicit human approval','non-production deployment','No fake approval','Phase 31')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Stage 17 approval missing: $_"}}
    $compose=Get-Content -Raw $stage17Paths.Compose;@('027_deployment_inputs.sql','028_deployment_evidence.sql','integration-demo-stage-17','deployment/seed.sql','--verification-key')|ForEach-Object{if($compose-notmatch[regex]::Escape($_)){throw "Stage 17 composition guard missing: $_"}}
    $policy=Get-Content -Raw $stage17Paths.Policy;@('internal-service.deployment.execute','artifactContentSha256Digest','deploymentProfileId','targetEnvironment','productionDeploymentRequested == "False"','humanApprovalReference','humanApprovalValid":true','aiAuthorityDetected":false','"outputKind":"deployment-result"','"Deny"')|ForEach-Object{if($policy-notmatch[regex]::Escape($_)){throw "Stage 17 policy guard missing: $_"}}
    $bundle=Get-Content -Raw $stage17Paths.BundleVerification;@('geaip-demo-stage-17','1717171717171717171717171717171717171717171717171717171717171717','trust://demo/opa/demo-opa-signing-01')|ForEach-Object{if($bundle-notmatch[regex]::Escape($_)){throw "Stage 17 bundle verification missing: $_"}}
    $preparation=Get-Content -Raw $stage17Paths.Preparation;@('--signing-key','accepted Artifact evidence','Artifact-stopped run and signed sovereign profile','sovereign non-production deployment gateway','Stage 17 infrastructure startup failed')|ForEach-Object{if($preparation-notmatch[regex]::Escape($_)){throw "Stage 17 preparation guard missing: $_"}}
    $runbook=Get-Content -Raw $stage17Paths.Runbook;@('IInstitutionalSovereignDeploymentGateway','Repository defaults remain empty and fail closed','never substitute a fake approval','DeploymentOccurred: true','ProductionEffectOccurred: false','TelemetryConfigured: false','CanAdvance: false','Public or production deployment is prohibited')|ForEach-Object{if($runbook-notmatch[regex]::Escape($_)){throw "Stage 17 runbook guard missing: $_"}}
    $page=Get-Content -Raw $stage17Paths.Page;@('Deploy governed non-production service','DeployAsync','operator.internal-service.deployment.execute','DeploymentHumanApprovalReference','ProductionEffectOccurred: false','TelemetryConfigured: false','CanAdvance: false')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 17 UI guard missing: $_"}}
    $realm=Get-Content -Raw $stage17Paths.Realm;@('developer.internal-service.artifact.publish','operator.internal-service.deployment.execute','demo.human-reviewer')|ForEach-Object{if($realm-notmatch[regex]::Escape($_)){throw "Stage 17 identity guard missing: $_"}}
    $acceptance=Get-Content -Raw $stage17AcceptancePath;@('CR-039','exact accepted Artifact evidence','No fake approval','DeploymentOccurred: true','ProductionEffectOccurred: false','TelemetryConfigured: false','CanAdvance: false','OpenTelemetry and all later stages remain outside this gate')|ForEach-Object{if($acceptance-notmatch[regex]::Escape($_)){throw "Stage 17 acceptance missing: $_"}}
}

$stage18AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_18_GOVERNED_OPENTELEMETRY_ACCEPTANCE.md'
if (Test-Path $stage18AcceptancePath) {
    $stage18Paths = @{ Change=Join-Path $repositoryRoot 'docs\change-control\CR-040-INTEGRATION-DEMO-GOVERNED-OPENTELEMETRY.md'; Compose=Join-Path $repositoryRoot 'deploy\demo\stage-18\compose.yml'; Policy=Join-Path $repositoryRoot 'deploy\demo\stage-18\opa\decision.rego'; BundleVerification=Join-Path $repositoryRoot 'deploy\demo\stage-18\opa\bundle_verification.rego'; Preparation=Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-18.ps1'; Runbook=Join-Path $repositoryRoot 'deploy\demo\stage-18\README.md'; Page=Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'; Realm=Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json' }
    $stage18Paths.Values|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){throw "Stage 18 artifact missing: $_"}}
    $change=Get-Content -Raw $stage18Paths.Change;@('Approved for bounded implementation','strict redaction','trusted local collector','No public exporter','Phase 31')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Stage 18 approval missing: $_"}}
    $compose=Get-Content -Raw $stage18Paths.Compose;@('029_opentelemetry_inputs.sql','030_opentelemetry_evidence.sql','integration-demo-stage-18','opentelemetry/seed.sql','--verification-key')|ForEach-Object{if($compose-notmatch[regex]::Escape($_)){throw "Stage 18 composition guard missing: $_"}}
    $policy=Get-Content -Raw $stage18Paths.Policy;@('internal-service.opentelemetry.activate','telemetryProfileId','requiredSignals == "Traces,Metrics,Logs"','redactionPolicySha256Digest','registrationAllowed":false','enterpriseModelMutationAllowed":false','"outputKind":"opentelemetry-result"','"Deny"')|ForEach-Object{if($policy-notmatch[regex]::Escape($_)){throw "Stage 18 policy guard missing: $_"}}
    $bundle=Get-Content -Raw $stage18Paths.BundleVerification;@('geaip-demo-stage-18','1818181818181818181818181818181818181818181818181818181818181818','trust://demo/opa/demo-opa-signing-01')|ForEach-Object{if($bundle-notmatch[regex]::Escape($_)){throw "Stage 18 bundle verification missing: $_"}}
    $preparation=Get-Content -Raw $stage18Paths.Preparation;@('--signing-key','accepted Deployment evidence and Deployment-stopped run','signed OpenTelemetry profile and redaction policy','OpenTelemetry gateway','Stage 18 infrastructure startup failed')|ForEach-Object{if($preparation-notmatch[regex]::Escape($_)){throw "Stage 18 preparation guard missing: $_"}}
    $runbook=Get-Content -Raw $stage18Paths.Runbook;@('IInstitutionalOpenTelemetryGateway','Repository defaults remain empty and fail closed','never substitutes a fake profile','TelemetryConfigured: true','ProductionEffectOccurred: false','AutomaticRegistrationOccurred: false','CanAdvance: false','Public exporters')|ForEach-Object{if($runbook-notmatch[regex]::Escape($_)){throw "Stage 18 runbook guard missing: $_"}}
    $page=Get-Content -Raw $stage18Paths.Page;@('Activate governed OpenTelemetry','ActivateTelemetryAsync','operator.internal-service.telemetry.activate','GovernedTelemetrySignal.Traces','TelemetryConfigured: true','AutomaticRegistrationOccurred: false','CanAdvance: false')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 18 UI guard missing: $_"}}
    $realm=Get-Content -Raw $stage18Paths.Realm;@('operator.internal-service.deployment.execute','operator.internal-service.telemetry.activate','demo.human-reviewer')|ForEach-Object{if($realm-notmatch[regex]::Escape($_)){throw "Stage 18 identity guard missing: $_"}}
    $acceptance=Get-Content -Raw $stage18AcceptancePath;@('CR-040','exact accepted Deployment evidence','No fake profile','TelemetryConfigured: true','ProductionEffectOccurred: false','AutomaticRegistrationOccurred: false','CanAdvance: false','Automatic Registration and all later stages remain outside this gate')|ForEach-Object{if($acceptance-notmatch[regex]::Escape($_)){throw "Stage 18 acceptance missing: $_"}}
}

$stage19AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_19_GOVERNED_AUTOMATIC_REGISTRATION_ACCEPTANCE.md'
if (Test-Path $stage19AcceptancePath) {
    $stage19Paths = @{ Change=Join-Path $repositoryRoot 'docs\change-control\CR-041-INTEGRATION-DEMO-GOVERNED-AUTOMATIC-REGISTRATION.md'; Compose=Join-Path $repositoryRoot 'deploy\demo\stage-19\compose.yml'; Policy=Join-Path $repositoryRoot 'deploy\demo\stage-19\opa\decision.rego'; BundleVerification=Join-Path $repositoryRoot 'deploy\demo\stage-19\opa\bundle_verification.rego'; Preparation=Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-19.ps1'; Runbook=Join-Path $repositoryRoot 'deploy\demo\stage-19\README.md'; Page=Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'; Realm=Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json' }
    $stage19Paths.Values|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){throw "Stage 19 artifact missing: $_"}}
    $change=Get-Content -Raw $stage19Paths.Change;@('Approved for bounded implementation','Phase 17 engine','signed deployment-controlled manifest','No workflow advancement','Phase 31')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Stage 19 approval missing: $_"}}
    $compose=Get-Content -Raw $stage19Paths.Compose;@('031_automatic_registration_inputs.sql','032_automatic_registration_evidence.sql','integration-demo-stage-19','automatic-registration/seed.sql','--verification-key')|ForEach-Object{if($compose-notmatch[regex]::Escape($_)){throw "Stage 19 composition guard missing: $_"}}
    $policy=Get-Content -Raw $stage19Paths.Policy;@('internal-service.automatic-registration.execute','manifestSha256Digest','serviceIdentity','openTelemetryEvidenceReference','registrationAllowed":true','workflowAdvancementAllowed":false','"outputKind":"automatic-registration-result"','"Deny"')|ForEach-Object{if($policy-notmatch[regex]::Escape($_)){throw "Stage 19 policy guard missing: $_"}}
    $bundle=Get-Content -Raw $stage19Paths.BundleVerification;@('geaip-demo-stage-19','1919191919191919191919191919191919191919191919191919191919191919','trust://demo/opa/demo-opa-signing-01')|ForEach-Object{if($bundle-notmatch[regex]::Escape($_)){throw "Stage 19 bundle verification missing: $_"}}
    $preparation=Get-Content -Raw $stage19Paths.Preparation;@('--signing-key','accepted OpenTelemetry evidence and OpenTelemetry-stopped run','signed Automatic Registration manifest','atomic repository','Stage 19 infrastructure startup failed')|ForEach-Object{if($preparation-notmatch[regex]::Escape($_)){throw "Stage 19 preparation guard missing: $_"}}
    $runbook=Get-Content -Raw $stage19Paths.Runbook;@('Phase 17 registration engine','Repository defaults remain empty and fail closed','never substitutes a fake manifest','AutomaticRegistrationOccurred: true','EnterpriseModelObjectPersisted: true','WorkflowAdvanced: false','CanAdvance: false')|ForEach-Object{if($runbook-notmatch[regex]::Escape($_)){throw "Stage 19 runbook guard missing: $_"}}
    $page=Get-Content -Raw $stage19Paths.Page;@('Execute governed Automatic Registration','RunAutomaticRegistrationAsync','operator.internal-service.registration.execute','AutomaticRegistrationOccurred: true','EnterpriseModelObjectPersisted: true','WorkflowAdvanced: false','CanAdvance: false')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 19 UI guard missing: $_"}}
    $realm=Get-Content -Raw $stage19Paths.Realm;@('operator.internal-service.telemetry.activate','operator.internal-service.registration.execute','demo.human-reviewer')|ForEach-Object{if($realm-notmatch[regex]::Escape($_)){throw "Stage 19 identity guard missing: $_"}}
    $acceptance=Get-Content -Raw $stage19AcceptancePath;@('CR-041','exact accepted OpenTelemetry evidence','No fake manifest','AutomaticRegistrationOccurred: true','EnterpriseModelObjectPersisted: true','WorkflowAdvanced: false','CanAdvance: false','Enterprise Model contextualization and all later stages remain outside this gate')|ForEach-Object{if($acceptance-notmatch[regex]::Escape($_)){throw "Stage 19 acceptance missing: $_"}}
}

$stage20AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_20_GOVERNED_ENTERPRISE_MODEL_ACCEPTANCE.md'
if (Test-Path $stage20AcceptancePath) {
    $stage20Paths = @{ Change=Join-Path $repositoryRoot 'docs\change-control\CR-042-INTEGRATION-DEMO-GOVERNED-ENTERPRISE-MODEL.md'; Compose=Join-Path $repositoryRoot 'deploy\demo\stage-20\compose.yml'; Policy=Join-Path $repositoryRoot 'deploy\demo\stage-20\opa\decision.rego'; BundleVerification=Join-Path $repositoryRoot 'deploy\demo\stage-20\opa\bundle_verification.rego'; Preparation=Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-20.ps1'; Runbook=Join-Path $repositoryRoot 'deploy\demo\stage-20\README.md'; Page=Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'; Realm=Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json' }
    $stage20Paths.Values|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){throw "Stage 20 artifact missing: $_"}}
    $change=Get-Content -Raw $stage20Paths.Change;@('Approved for bounded implementation','authorized exact-object reader','No new model mutation','Evidence completion','Phase 31')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Stage 20 approval missing: $_"}}
    $compose=Get-Content -Raw $stage20Paths.Compose;@('033_enterprise_model_inputs.sql','034_enterprise_model_evidence.sql','integration-demo-stage-20','enterprise-model/seed.sql','--verification-key')|ForEach-Object{if($compose-notmatch[regex]::Escape($_)){throw "Stage 20 composition guard missing: $_"}}
    $policy=Get-Content -Raw $stage20Paths.Policy;@('internal-service.enterprise-model.contextualize','enterpriseObjectId','requestFingerprint','registrationEvidenceReference','objectReadAllowed":true','mutationAllowed":false','workflowAdvancementAllowed":false','"outputKind":"enterprise-model-context-result"','"Deny"')|ForEach-Object{if($policy-notmatch[regex]::Escape($_)){throw "Stage 20 policy guard missing: $_"}}
    $bundle=Get-Content -Raw $stage20Paths.BundleVerification;@('geaip-demo-stage-20','2020202020202020202020202020202020202020202020202020202020202020','trust://demo/opa/demo-opa-signing-01')|ForEach-Object{if($bundle-notmatch[regex]::Escape($_)){throw "Stage 20 bundle verification missing: $_"}}
    $preparation=Get-Content -Raw $stage20Paths.Preparation;@('--signing-key','accepted Automatic Registration evidence and stopped run','authorized Enterprise Object','Enterprise Model reader','Stage 20 infrastructure startup failed')|ForEach-Object{if($preparation-notmatch[regex]::Escape($_)){throw "Stage 20 preparation guard missing: $_"}}
    $runbook=Get-Content -Raw $stage20Paths.Runbook;@('authorized exact Enterprise Object read','Repository defaults remain empty and fail closed','never substitutes a fake object','EnterpriseModelContextualized: true','EnterpriseModelMutated: false','EvidenceCompleted: false','CanAdvance: false')|ForEach-Object{if($runbook-notmatch[regex]::Escape($_)){throw "Stage 20 runbook guard missing: $_"}}
    $page=Get-Content -Raw $stage20Paths.Page;@('Contextualize governed Enterprise Model','ContextualizeEnterpriseModelAsync','operator.internal-service.enterprise-model.contextualize','EnterpriseModelContextualized: true','EnterpriseModelMutated: false','EvidenceCompleted: false','CanAdvance: false')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 20 UI guard missing: $_"}}
    $realm=Get-Content -Raw $stage20Paths.Realm;@('operator.internal-service.registration.execute','operator.internal-service.enterprise-model.contextualize','demo.human-reviewer')|ForEach-Object{if($realm-notmatch[regex]::Escape($_)){throw "Stage 20 identity guard missing: $_"}}
    $acceptance=Get-Content -Raw $stage20AcceptancePath;@('CR-042','exact accepted Automatic Registration evidence','No fake object','EnterpriseModelContextualized: true','EnterpriseModelMutated: false','EvidenceCompleted: false','CanAdvance: false','Evidence completion remains outside this gate')|ForEach-Object{if($acceptance-notmatch[regex]::Escape($_)){throw "Stage 20 acceptance missing: $_"}}
}

$stage21AcceptancePath = Join-Path $repositoryRoot 'docs\demo\STAGE_21_GOVERNED_EVIDENCE_COMPLETION_ACCEPTANCE.md'
if (Test-Path $stage21AcceptancePath) {
    $stage21Paths = @{ Change=Join-Path $repositoryRoot 'docs\change-control\CR-043-INTEGRATION-DEMO-GOVERNED-EVIDENCE-COMPLETION.md'; Compose=Join-Path $repositoryRoot 'deploy\demo\stage-21\compose.yml'; Policy=Join-Path $repositoryRoot 'deploy\demo\stage-21\opa\decision.rego'; BundleVerification=Join-Path $repositoryRoot 'deploy\demo\stage-21\opa\bundle_verification.rego'; Preparation=Join-Path $repositoryRoot 'scripts\prepare-integration-demo-stage-21.ps1'; Runbook=Join-Path $repositoryRoot 'deploy\demo\stage-21\README.md'; Page=Join-Path $repositoryRoot 'frontend\Platform.Web\Pages\InternalService.razor'; Realm=Join-Path $repositoryRoot 'deploy\demo\keycloak\geaip-demo-realm.json' }
    $stage21Paths.Values|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){throw "Stage 21 artifact missing: $_"}}
    $change=Get-Content -Raw $stage21Paths.Change;@('Approved for bounded implementation','Phase 30 engine','ten ordered stages','No update/deletion','Phase 31')|ForEach-Object{if($change-notmatch[regex]::Escape($_)){throw "Stage 21 approval missing: $_"}}
    $compose=Get-Content -Raw $stage21Paths.Compose;@('035_evidence_completion_inputs.sql','036_evidence_chain.sql','integration-demo-stage-21','evidence/seed.sql','--verification-key')|ForEach-Object{if($compose-notmatch[regex]::Escape($_)){throw "Stage 21 composition guard missing: $_"}}
    $policy=Get-Content -Raw $stage21Paths.Policy;@('internal-service.evidence.complete','chainId','correlationId','payloadSha256Digest','traceReferences','appendFinalEvidenceAllowed":true','verificationAllowed":true','workflowAdvancementAllowed":false','"outputKind":"evidence-completion-result"','"Deny"')|ForEach-Object{if($policy-notmatch[regex]::Escape($_)){throw "Stage 21 policy guard missing: $_"}}
    $bundle=Get-Content -Raw $stage21Paths.BundleVerification;@('geaip-demo-stage-21','2121212121212121212121212121212121212121212121212121212121212121','trust://demo/opa/demo-opa-signing-01')|ForEach-Object{if($bundle-notmatch[regex]::Escape($_)){throw "Stage 21 bundle verification missing: $_"}}
    $preparation=Get-Content -Raw $stage21Paths.Preparation;@('--signing-key','accepted Enterprise Model contextualization and stopped run','nine-entry evidence chain headed by Telemetry','sovereign signing key','Stage 21 infrastructure startup failed')|ForEach-Object{if($preparation-notmatch[regex]::Escape($_)){throw "Stage 21 preparation guard missing: $_"}}
    $runbook=Get-Content -Raw $stage21Paths.Runbook;@('Phase 30 append-only atomic chain store','Repository defaults contain no keys and fail closed','never substitutes fake chain entries','EvidenceAppended: true','EvidenceCryptographicallyVerified: true','VerticalSliceComplete: true','VerifiedEntryCount: 10','No post-Evidence station')|ForEach-Object{if($runbook-notmatch[regex]::Escape($_)){throw "Stage 21 runbook guard missing: $_"}}
    $page=Get-Content -Raw $stage21Paths.Page;@('Integration Demo Stage 21','Complete governed Evidence chain','CompleteEvidenceAsync','operator.internal-service.evidence.complete','evidence.append','evidence.verify','EvidenceAppended: true','EvidenceCryptographicallyVerified: true','VerticalSliceComplete: true','VerifiedEntryCount: 10','geaip-demo-stage-21')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 21 UI guard missing: $_"}}
    $realm=Get-Content -Raw $stage21Paths.Realm;@('operator.internal-service.evidence.complete','evidence.append','evidence.verify','demo.human-reviewer')|ForEach-Object{if($realm-notmatch[regex]::Escape($_)){throw "Stage 21 identity guard missing: $_"}}
