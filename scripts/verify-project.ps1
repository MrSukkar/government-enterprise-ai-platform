...
if (Test-Path $stage20AcceptancePath) {
    ...
    $page=Get-Content -Raw $stage20Paths.Page;@('Integration Demo Stage 20','Contextualize governed Enterprise Model','ContextualizeEnterpriseModelAsync','operator.internal-service.enterprise-model.contextualize','EnterpriseModelContextualized: true','EnterpriseModelMutated: false','EvidenceCompleted: false','CanAdvance: false','geaip-demo-stage-20')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 20 UI guard missing: $_"}}
