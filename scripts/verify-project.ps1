...
if (Test-Path $stage19AcceptancePath) {
    ...
    $page=Get-Content -Raw $stage19Paths.Page;@('Integration Demo Stage 19','Execute governed Automatic Registration','RunAutomaticRegistrationAsync','operator.internal-service.registration.execute','AutomaticRegistrationOccurred: true','EnterpriseModelObjectPersisted: true','WorkflowAdvanced: false','CanAdvance: false','geaip-demo-stage-19')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 19 UI guard missing: $_"}}
