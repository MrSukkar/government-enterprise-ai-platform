...
if (Test-Path $stage17AcceptancePath) {
    ...
    $page=Get-Content -Raw $stage17Paths.Page;@('Integration Demo Stage 17','Deploy governed non-production service','DeployAsync','operator.internal-service.deployment.execute','DeploymentHumanApprovalReference','ProductionEffectOccurred: false','TelemetryConfigured: false','CanAdvance: false','geaip-demo-stage-17')|ForEach-Object{if($page-notmatch[regex]::Escape($_)){throw "Stage 17 UI guard missing: $_"}}
