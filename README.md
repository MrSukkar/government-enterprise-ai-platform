...
- The first business implementation, **Create Internal Service Workspace**, is authorized under CR-001 and Amendment 01 through Operational Increment 04. Authorized Enterprise Context discovery is protected, bound to a registered intent, OPA-scoped before source access, per-result re-authorized, deterministic, and evidence-bearing. Deployment-controlled registration-read, OPA, retrieval-source, and evidence adapters remain unavailable, so source access, institutional mutation, advancement, AI, and material execution fail closed.
