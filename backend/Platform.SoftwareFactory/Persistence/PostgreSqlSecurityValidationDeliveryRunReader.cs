...
public sealed class PostgreSqlSecurityValidationDeliveryRunReader(
    NpgsqlDataSource dataSource, IOptions<PostgreSqlIntentRegistrationOptions> configuredOptions)
    : ISecurityValidationDeliveryRunReader
{
    private const string SelectSql = """
        SELECT record_json, record_sha256_digest, evidence_reference, recorded_at
          FROM software_factory.security_validation_delivery_run_snapshots
         WHERE tenant_id = @tenant_id AND run_id = @run_id AND purpose = @purpose
           AND generation_id = @generation_id AND static_validation_id = @static_validation_id
           AND candidate_sha256_digest = @candidate_sha256_digest
           AND static_report_sha256_digest = @static_report_sha256_digest
        """;
    ...
    {
        if (runId == Guid.Empty || generationId == Guid.Empty || staticValidationId == Guid.Empty)
            throw new ArgumentException("Security run identities are required.");
        ...
        GovernedAiPlanningRequest.ValidateDigest(staticReportSha256Digest, "Static report"); var options = configuredOptions.Value;
        if (!options.IsOperationallyConfigured)
            throw new SecurityValidationDependencyUnavailableException("Security delivery-run snapshots are not configured.");
        try
        {
            ...
            Add(command, "static_report_sha256_digest", NpgsqlDbType.Text, staticReportSha256Digest.ToLowerInvariant());
            ...
            var run = JsonSerializer.Deserialize<SoftwareDeliveryRun>(json) ??
                throw new InvalidOperationException("Stored Security run is empty.");
            ...
            if (run.Id != runId || !StringComparer.Ordinal.Equals(run.TenantId, tenantId) ||
                ...
                run.CurrentStage != DeliveryStage.StaticValidation || run.History.IsDefaultOrEmpty ||
                run.History.Any(item => string.IsNullOrWhiteSpace(item.EvidenceReference) || item.CompletedAt > recordedAt))
                throw new InvalidOperationException("Stored Security run failed exact binding validation.");
            return run;
        }
        catch (NpgsqlException) { throw new SecurityValidationDependencyUnavailableException("Security delivery-run read is unavailable."); }
        catch (TimeoutException) { throw new SecurityValidationDependencyUnavailableException("Security delivery-run read timed out."); }
        catch (JsonException exception) { throw new InvalidOperationException("Stored Security run is malformed.", exception); }
    }
