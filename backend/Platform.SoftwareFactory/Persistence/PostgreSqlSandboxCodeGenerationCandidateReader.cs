...
public sealed class PostgreSqlSandboxCodeGenerationCandidateReader(
    ...
    : ISandboxCodeGenerationCandidateReader
{
    private const string SelectSql = """
        SELECT generation.record_json, generation.record_sha256_digest,
          ...
          JOIN software_factory.security_validation_evidence AS security
            ON security.tenant_id = generation.tenant_id AND security.generation_id = generation.generation_id
         WHERE generation.tenant_id = @tenant_id AND generation.generation_id = @generation_id
           ...
           AND security.validation_id = @security_validation_id
           AND security.candidate_sha256_digest = @candidate_sha256_digest
           AND security.evidence_reference = @security_evidence_reference
        """;

    public async Task<AuthorizedCodeGenerationCandidateSnapshot?> LoadAsync(Guid generationId,
        string tenantId, string purpose, string candidateSha256Digest, Guid securityValidationId,
        string securityEvidenceReference, CancellationToken cancellationToken)
    {
        if (generationId == Guid.Empty || securityValidationId == Guid.Empty)
            throw new ArgumentException("Sandbox candidate identities are required.");
        ...
        if (!options.IsOperationallyConfigured)
            throw new SandboxDependencyUnavailableException("PostgreSQL Code Generation evidence is not configured.");
        try
        {
            ...
            Add(command, "security_evidence_reference", NpgsqlDbType.Text, securityEvidenceReference);
            ...
            if (record.GenerationId != generationId || !StringComparer.Ordinal.Equals(record.TenantId, tenantId) ||
                ...
                throw new InvalidOperationException("Stored Code Generation evidence failed Sandbox-chain validation.");
        ...
        catch (NpgsqlException) { throw new SandboxDependencyUnavailableException("Code Generation evidence read is unavailable."); }
        catch (TimeoutException) { throw new SandboxDependencyUnavailableException("Code Generation evidence read timed out."); }
