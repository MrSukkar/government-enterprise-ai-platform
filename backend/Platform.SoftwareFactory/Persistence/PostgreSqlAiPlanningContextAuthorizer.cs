...
public sealed class PostgreSqlAiPlanningContextAuthorizer(
    NpgsqlDataSource dataSource,
    IOptions<PostgreSqlIntentRegistrationOptions> configuredOptions,
    IAccessPolicyEvaluator accessPolicyEvaluator) : IAiPlanningContextAuthorizer
{
    private const string SelectSql = """
        SELECT source_station, record_json, record_sha256_digest, evidence_reference,
               evidence_references, recorded_at
          FROM (
            SELECT 'enterprise-context' AS source_station, record_json, record_sha256_digest,
                   evidence_reference, evidence_references, recorded_at
              FROM software_factory.enterprise_context_evidence
             WHERE tenant_id = @tenant_id AND evidence_reference = @evidence_reference
            UNION ALL
            SELECT 'existing-systems', record_json, record_sha256_digest,
                   evidence_reference, evidence_references, recorded_at
              FROM software_factory.existing_systems_evidence
             WHERE tenant_id = @tenant_id AND evidence_reference = @evidence_reference
            UNION ALL
            SELECT 'existing-architecture', record_json, record_sha256_digest,
                   evidence_reference, evidence_references, recorded_at
              FROM software_factory.existing_architecture_evidence
             WHERE tenant_id = @tenant_id AND evidence_reference = @evidence_reference
            UNION ALL
            SELECT 'approved-packages', record_json, record_sha256_digest,
                   evidence_reference, evidence_references, recorded_at
    ...
    public async Task<AiPlanningContextAuthorizationDecision> AuthorizeAsync(
        AiPlanningContextAuthorizationRequest request, CancellationToken cancellationToken)
    {
        ValidateRequest(request); var options = configuredOptions.Value;
        if (!options.IsOperationallyConfigured)
            throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning context is not configured.");
        try
        {
            ...
            if (!await reader.ReadAsync(cancellationToken))
                throw new UnauthorizedAccessException("Authorized AI Planning context material was not found.");
            ...
            if (await reader.ReadAsync(cancellationToken))
                throw new InvalidOperationException("AI Planning context reference resolved to multiple records.");
            ...
            if (!StringComparer.Ordinal.Equals(reference, request.ContextReference) ||
                ...
                storedEvidence.IsDefaultOrEmpty || material.Classification > request.MaximumClassification)
                throw new UnauthorizedAccessException("AI Planning context material failed exact binding validation.");
            var access = accessPolicyEvaluator.Evaluate(new AccessRequest(
                request.Identity, request.Purpose, "ai-context.read", reference, request.TenantId,
                material.Classification, request.RequiredRoles,
            ...
            var authorizationDigest = Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes(string.Join('|',
                ...
                request.SubjectId, request.Purpose, request.Environment, reference, digest, material.Classification,
                access.IsAllowed, access.Code))));
            var evidence = request.EvidenceReferences.Concat(storedEvidence).Append(reference)
                .Append($"evidence://ai-planning/context-authorization/{request.AuthorizationRequestId:D}/sha256/{authorizationDigest}")
                .Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal).ToImmutableArray();
            var item = new AiDevelopmentContextItem(reference, station, material.Classification,
                digest, material.Content, evidence);
            return new(request.AuthorizationRequestId, request.PlanningId, request.TenantId, reference,
                access.IsAllowed, access.Code, access.IsAllowed ? item : null, evidence, request.RequestedAt);
        }
        catch (NpgsqlException) { throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning context read is unavailable."); }
        catch (TimeoutException) { throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning context read timed out."); }
        catch (JsonException exception) { throw new InvalidOperationException("Authorized AI Planning context is malformed.", exception); }
    }

    private static (string Content, DataClassification Classification) Parse(
        string station, string json, AiPlanningContextAuthorizationRequest request, DateTimeOffset recordedAt)
    {
        return station switch
        {
            ...
            _ => throw new UnauthorizedAccessException("AI Planning context source is not approved.")
        };
    }

    private static (string, DataClassification) Normalize(EnterpriseContextEvidenceRecord value,
        AiPlanningContextAuthorizationRequest request, DateTimeOffset recordedAt)
    {
        ValidateCommon(value.TenantId, value.Purpose, value.DiscoveredAt, value.EvidenceReferences, request, recordedAt);
        return (JsonSerializer.Serialize(value), value.Items.Max(item => item.Classification));
    }
    private static (string, DataClassification) Normalize(ExistingSystemsEvidenceRecord value,
        AiPlanningContextAuthorizationRequest request, DateTimeOffset recordedAt)
    {
        ValidateCommon(value.TenantId, value.Purpose, value.DiscoveredAt, value.EvidenceReferences, request, recordedAt);
        return (JsonSerializer.Serialize(value), value.Systems.Max(item => item.Classification));
    }
    ...
    private static (string, DataClassification) Normalize(ApprovedPackagesEvidenceRecord value,
        AiPlanningContextAuthorizationRequest request, DateTimeOffset recordedAt)
    {
        ValidateCommon(value.TenantId, value.Purpose, value.SelectedAt, value.EvidenceReferences, request, recordedAt);
    ...
    private static void ValidateCommon(string tenantId, string purpose, DateTimeOffset createdAt,
        ImmutableArray<string> evidence, AiPlanningContextAuthorizationRequest request, DateTimeOffset recordedAt)
    {
        if (!StringComparer.Ordinal.Equals(tenantId, request.TenantId) ||
            !StringComparer.Ordinal.Equals(purpose, request.Purpose) || createdAt == default ||
            createdAt > recordedAt || evidence.IsDefaultOrEmpty)
            throw new UnauthorizedAccessException("AI Planning context scope is invalid.");
    }

    private static void ValidateRequest(AiPlanningContextAuthorizationRequest request)
    {
        if (request.AuthorizationRequestId == Guid.Empty || request.PlanningId == Guid.Empty ||
            ...
            throw new InvalidOperationException("AI Planning context authorization request is incomplete.");
        ...
        if (!request.Identity.IsAuthenticated || !StringComparer.Ordinal.Equals(request.Identity.SubjectId, request.SubjectId) ||
            ...
            throw new UnauthorizedAccessException("AI Planning context authorization identity is mismatched.");
