...
public sealed class PostgreSqlAiPlanningDeliveryRunReader(
    NpgsqlDataSource dataSource, IOptions<PostgreSqlIntentRegistrationOptions> configuredOptions)
    : IAiPlanningDeliveryRunReader
{
    private const string SelectSql = """
        SELECT record_json, record_sha256_digest, evidence_reference, recorded_at
         FROM software_factory.ai_planning_delivery_run_snapshots
         WHERE tenant_id = @tenant_id AND run_id = @run_id
           AND package_selection_id = @package_selection_id
           AND selection_sha256_digest = @selection_sha256_digest
           AND purpose = @purpose
        """;

    public async Task<SoftwareDeliveryRun?> LoadAsync(
    ...
    {
        if (runId == Guid.Empty) throw new ArgumentException("Delivery run identity is required.");
        if (packageSelectionId == Guid.Empty) throw new ArgumentException("Package selection identity is required.");
        ...
        if (!options.IsOperationallyConfigured)
            throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning delivery-run snapshots are not configured.");
        try
        {
            ...
            Add(command, "purpose", NpgsqlDbType.Text, purpose);
            ...
            var run = JsonSerializer.Deserialize<SoftwareDeliveryRun>(json)
                ?? throw new InvalidOperationException("Stored delivery-run snapshot is empty.");
            ...
            if (run.Id != runId || !StringComparer.Ordinal.Equals(run.TenantId, tenantId) ||
                ...
                run.History.IsDefaultOrEmpty || run.CurrentStage != DeliveryStage.ApprovedPackages ||
                run.History.Any(item => string.IsNullOrWhiteSpace(item.EvidenceReference) || item.CompletedAt > recordedAt))
                throw new InvalidOperationException("Stored delivery-run snapshot failed exact binding validation.");
            return run;
        }
        catch (NpgsqlException) { throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning delivery-run read is unavailable."); }
        catch (TimeoutException) { throw new AiPlanningDependencyUnavailableException("PostgreSQL AI Planning delivery-run read timed out."); }
        catch (JsonException exception) { throw new InvalidOperationException("Stored delivery-run snapshot is malformed.", exception); }
    }
