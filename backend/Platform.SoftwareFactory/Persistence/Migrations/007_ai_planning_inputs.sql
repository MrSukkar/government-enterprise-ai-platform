...
CREATE TABLE IF NOT EXISTS software_factory.ai_planning_delivery_run_snapshots
(
    ...
    CONSTRAINT pk_ai_planning_delivery_run_snapshots PRIMARY KEY (tenant_id, run_id),
    CONSTRAINT uq_ai_planning_delivery_run_evidence UNIQUE (tenant_id, evidence_reference),
    CONSTRAINT fk_ai_planning_delivery_run_packages FOREIGN KEY (tenant_id, package_selection_id)
        REFERENCES software_factory.approved_packages_evidence (tenant_id, selection_id) ON UPDATE RESTRICT ON DELETE RESTRICT
);

CREATE TABLE IF NOT EXISTS software_factory.governed_planning_prompt_templates
(
    ...
    is_planning_approved boolean NOT NULL,
    ...
    CONSTRAINT pk_governed_planning_prompt_templates PRIMARY KEY (template_id, template_version)
);
