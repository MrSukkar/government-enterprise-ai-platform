    ...
    : ISandboxDeliveryRunReader
{
    ...
    {
        if (runId == Guid.Empty || generationId == Guid.Empty || securityValidationId == Guid.Empty)
            throw new ArgumentException("Sandbox run identities are required.");
        ...
        GovernedAiPlanningRequest.ValidateDigest(securityReportSha256Digest, "Security report"); var options = configuredOptions.Value;
        if (!options.IsOperationallyConfigured)
            throw new SandboxDependencyUnavailableException("Sandbox delivery-run snapshots are not configured.");
        try
        {
            ...
            Add(command, "security_validation_id", NpgsqlDbType.Uuid, securityValidationId);
            ...
            var run = JsonSerializer.Deserialize<SoftwareDeliveryRun>(json)
                ?? throw new InvalidOperationException("Stored Sandbox run is empty.");
            ...
            if (run.Id != runId || !StringComparer.Ordinal.Equals(run.TenantId, tenantId) ||
                ...
                run.CurrentStage != DeliveryStage.SecurityValidation || run.History.IsDefaultOrEmpty ||
                run.History.Any(item => string.IsNullOrWhiteSpace(item.EvidenceReference) || item.CompletedAt > recordedAt))
                throw new InvalidOperationException("Stored Sandbox run failed exact binding validation.");
            return run;
        }
        catch (NpgsqlException) { throw new SandboxDependencyUnavailableException("Sandbox delivery-run read is unavailable."); }
        catch (TimeoutException) { throw new SandboxDependencyUnavailableException("Sandbox delivery-run read timed out."); }
        catch (JsonException exception) { throw new InvalidOperationException("Stored Sandbox run is malformed.", exception); }
    }
